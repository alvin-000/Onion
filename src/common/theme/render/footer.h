...
// Top of the hint bar. In design space this is y=420; deriving it from the
// panel instead keeps the bar flush with the bottom edge on any screen.
#define THEME_FOOTER_TOP (g_display.height - theme_scaleY(THEME_FOOTER_HEIGHT))

void theme_renderStandardHint(SDL_Surface *screen, const char *btn_a_str,
                              const char *btn_b_str)
{
    // Design-space y=450 and y=449 respectively; the one-pixel difference is
    // preserved so 640x480 output is unchanged.
    const int hint_center_y = THEME_FOOTER_TOP + theme_scaleY(30);
    const int label_center_y = THEME_FOOTER_TOP + theme_scaleY(29);

    int offsetX = theme_scaleX(20);
    ...
    SDL_Rect btn_a_rect = {offsetX, hint_center_y};
    SDL_Rect label_open_rect = {0, label_center_y};
    ...
    if (label_open) {
        ...
        offsetX += label_open->w + theme_scaleX(30);
    }

    if (btn_b_str != NULL && strlen(label_b_str) > 0) {
        SDL_Surface *button_b = resource_getSurface(BUTTON_B);
        SDL_Rect btn_b_rect = {offsetX, hint_center_y - button_b->h / 2};
        ...
        if (label_back) {
            SDL_Rect label_back_rect = {offsetX, label_center_y - label_back->h / 2};
...
{
    int footer_height = theme_scaleY(THEME_FOOTER_HEIGHT);
                              ...
                              int total_num)
{
    if (old_status_width != -1) {
        SDL_Rect status_pos = {g_display.width - theme_scaleX(20), THEME_FOOTER_TOP, 0,
                               theme_scaleY(THEME_FOOTER_HEIGHT)};
        status_pos.x -= old_status_width;
        status_pos.w = old_status_width;

        SDL_Rect status_size = {status_pos.x, 0, old_status_width, theme_scaleY(THEME_FOOTER_HEIGHT)};
    ...
    const int status_center_y = THEME_FOOTER_TOP + theme_scaleY(29);
    TTF_Font *font_hint = resource_getFont(HINT);
    ...
    if (total) {
        ...
        SDL_Rect total_rect = {g_display.width - theme_scaleX(20) - total->w,
                               status_center_y - total->h / 2};

        SDL_BlitSurface(total, NULL, screen, &total_rect);
        if (current) {
            current_rect.x = total_rect.x - current->w;
            current_rect.y = status_center_y - current->h / 2;
